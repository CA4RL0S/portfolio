  ...
  return (
    <nav style={{
      ...
      <a href="#hero" style={{
        ...
        <span style={{ opacity: 0.5 }}>&gt;_</span> carlos.dev
      </a>
      ...
      <ul style={{ display: 'flex', gap: '28px', listStyle: 'none', alignItems: 'center' }} className="nav-desktop">
        {links.map(l => (
          <li key={l}>
            <a href={`#${l.toLowerCase()}`} style={{
              ...
              letterSpacing: '0.12em', color: 'var(--white-dim)',
        ...
        <li style={{ width: '1px', height: '18px', background: 'var(--border)' }} />

        {socials.map(s => (
          <li key={s.name}>
            <a href={s.href} target="_blank" rel="noopener noreferrer" title={s.name}
              style={{ color: 'var(--white-muted)', display: 'flex', alignItems: 'center', transition: 'color 0.2s, transform 0.2s' }}
              onMouseEnter={e => { e.currentTarget.style.color = 'var(--cyan)'; e.currentTarget.style.transform = 'translateY(-2px)'; }}
              onMouseLeave={e => { e.currentTarget.style.color = 'var(--white-muted)'; e.currentTarget.style.transform = 'none'; }}
            >{s.icon}</a>
          </li>
        ))}
      </ul>
        ...
        <div style={{
          ...
          {links.map(l => (
            <a key={l} href={`#${l.toLowerCase()}`} onClick={() => setMenuOpen(false)}
              style={{ fontFamily: 'var(--font-mono)', fontSize: '13px', color: 'var(--white-dim)', letterSpacing: '0.15em', textTransform: 'uppercase' }}
            >{l}</a>
          ))}
        </div>
      )}
